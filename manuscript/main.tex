...
interactive application, the tutorial notebook, the simulation grids and the
scripts that produce every number and figure in this paper are in the project
repository, and both packages travel with the submission as source archives
(\code{sparse\_gene\_set\_calibration-0.1.0.tar.gz} and
\code{sparseGenSetCal\_0.1.0.tar.gz}). The claim that every table regenerates
...
workflow is defined in the repository (\code{.github/workflows/ci.yml}) but
has not yet run on a public host, so no CI status is claimed until the
repository is public and the badge is green. The archived release carries a
...
\paragraph{Continuous integration and archiving.} The CI workflow is defined
in the repository and will exercise the regeneration on a public host once
the repository is released; until then no CI status is claimed. The archived
